...
retail_ts <- rio::import("https://byuistats.github.io/timeseries/data/retail_by_business_type.parquet") |>
  filter(naics == 44812) |>
  mutate(month = yearmonth(as.Date(month, origin = "1970-01-01"))) |> # added this to fix error. this data is common for this type of month format
  mutate(month = yearmonth(month)) |> # added to fix error
...
retail_pred_df <- rio::import("data/retail_by_business_type.parquet") |>
  filter(naics == 44812) |>
  mutate(month = yearmonth(as.Date(month, origin = "1970-01-01"))) |>  # Convert month
  ...
  mutate(month_text = format(month, "%b %Y")) |>
  as_tibble() |>
  rename(t = month_seq) |> # Rename for clarity
  dplyr::select(month_text, month_num, t, sales_millions) |>
  mutate(
    alpha1_t = slope_retail * t,
    deviation = sales_millions - alpha1_t
  ...
  left_join(
    retail_df2 |> 
      dplyr::select(month_num, mean_deviation) |> # Properly select columns
      distinct(),  # Remove duplicates before joining
    by = "month_num"
  ) |>
  dplyr::select(month_text, t, sales_millions, alpha1_t, deviation, mean_deviation)
...
#  replace_na_with_char(emdash) |> remove this to show results
  rename(
    "$$Date$$" = month_text,
    "$$t$$" = t,
    # "$$i$$" = month_num,
...
retail_df3 |> 
  kableExtra::kable() |>
  kableExtra::column_spec(1:3, width_min = "0.35in") |>
  kableExtra::column_spec(4:ncol(retail_df3), width_min = "1in")
